    ...
    <section id="cabanas" className="bg-noche px-5 py-24 sm:px-8 md:py-32">
      <div className="mx-auto grid max-w-6xl items-center gap-16 lg:grid-cols-2 lg:gap-20">
        <Aparece>
          ...
          <p className="border-filete text-bruma-2 mt-8 inline-flex items-center gap-2.5 rounded-full border px-5 py-2.5 text-[0.9rem]">
            <Users size={16} className="text-sol" strokeWidth={1.9} />
            Hasta{' '}
            <strong className="text-bruma font-semibold">
      ...
      <div className="mx-auto mt-24 max-w-6xl md:mt-32">
        <Aparece>
          <p className="condensada text-musgo mb-4 text-[0.72rem]">Lo que hay</p>
          <h2 className="text-bruma max-w-lg text-[1.8rem] leading-[1.15] sm:text-[2.3rem]">
            Todo esto es de ustedes mientras estén.
          </h2>
        </Aparece>

        <div className="border-filete bg-filete mt-12 grid gap-px overflow-hidden rounded-2xl border sm:grid-cols-2 lg:grid-cols-3">
          {AMENIDADES.map(({icono: Icono, titulo, detalle}, i) => (
            <Aparece key={titulo} demora={(i % 3) * 0.06}>
              <div className="bg-bosque hover:bg-bosque-alto h-full p-6 transition-colors duration-300 lg:p-7">
                <Icono size={20} className="text-sol mb-4" strokeWidth={1.6} />
                <h3 className="text-bruma font-sans text-[1rem] font-semibold">{titulo}</h3>
                <p className="text-bruma-2 mt-1.5 text-[0.9rem] leading-relaxed">{detalle}</p>
              </div>
            </Aparece>
          ))}
        </div>
